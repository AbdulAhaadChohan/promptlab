...
### 3.4 Edge Cases in Suite Parsing
...
- `cases`: Array of case result objects
  ...
  - `failures`: Array containing failure information (empty if pass)
    - Each failure object includes:
      ...
      **For composite assertion failures (`all_of`, `any_of`):**
      - The top-level assertion object in the `assertions` section counts as 1 entry.
      - The `failures` array entry details the specific child assertion that caused the failure, including:
...
### 7.5 Comparison JSON Output Schema
The `promptlab compare` command generates the following JSON schema when comparing two reports:
```json
{
...
- `tokens_in_change_pct`: Percentage change in total tokens_in between reports.
- `tokens_out_change_pct`: Percentage change in total tokens_out between reports.
- `wall_ms_change_pct`: Percentage change in total wall_ms between reports.
...
- `unchanged`: Cases with equal `pass_rate` (within floating-point precision).
